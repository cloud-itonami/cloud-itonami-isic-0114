(ns caneops.phase
  "Phase gate for the sugar-cane plantation operations coordinator rollout
  (0->3 maturity). Mirrors `riceops.phase` (cloud-itonami-isic-0112) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`caneops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [caneops.governor :as governor]))
