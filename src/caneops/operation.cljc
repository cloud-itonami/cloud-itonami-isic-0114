(ns caneops.operation
  ...
  actor run. The advisor (CaneOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Cane Operations
  ...
  One run = one sugar-cane plantation coordination operation (intake ->
  advise -> govern -> decide -> commit | hold | approval). No unbounded
  inner loop -- each operation is auditable and checkpointed. A cane
  ...
  hands the decision to a human operator (grower / agronomist). The
  ...
  is clean -- see `caneops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `riceops.operation`, cloud-itonami-isic-0112). This stub version
  ...
  (:require [caneops.advisor :as advisor]
...
(defn run-operation
  "Run one sugar-cane plantation operation through the advisor -> governor
